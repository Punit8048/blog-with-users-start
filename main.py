...
class User(UserMixin, db.Model):
    ...
    comments = relationship("Comment", back_populates="comment_auther")
...
class Comment(db.Model):
    ...
    comment_auther = relationship("User", back_populates="comments")
...
def login():
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            flash("That password does not match, please try again.")
            return redirect(url_for('login'))
        else:
            login_user(user)
            return redirect(url_for('get_all_posts'))
...
def show_post(post_id):
    ...
    if request.method == "GET":
        if form.validate_on_submit():
            new_comment = Comment(text=form.comment_text.data, comment_auther=current_user)
            db.session.add(new_comment)
            db.session.commit()
            return redirect(url_for("show_post"))

    return render_template("post.html", comment=comment, form=form, post=requested_post, logged_in=current_user.is_authenticated)
